...
class SellFrame(TabFrame):
    def _init(self, *args, **kwargs) -> None:
        ...
        ttk.Label(master=self, text="Interval mins:", width=15).grid(row=2, column=0)
        ttk.Label(master=self, text="Return Spot:", width=15).grid(row=3, column=0)
        ...
        tk.Entry(master=self, textvariable=self._vars["interval_mins"], takefocus=False).grid(row=2, column=1)
        tk.Entry(master=self, textvariable=self._vars["return_spot"], takefocus=False).grid(row=3, column=1)
    ...
    def _set_spot_as_current(self):
        ...
        self._vars['return_spot'].set("Reading Mouse...")
        time.sleep(3)
        mouse_pos = ClientWindow.get_mouse_window_pos(window_pos)
        self._vars['return_spot'].set("{} {}".format(*mouse_pos))
