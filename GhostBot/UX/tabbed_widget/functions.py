...
class FunctionsFrame(tk.Frame):
    def __init__(self, master):
        ...
        ttk.Label(master=char_info_frame, text="HP:", width=15).grid(row=2, column=0)
        ttk.Label(master=char_info_frame, text="Mana:", width=15).grid(row=3, column=0)
        ttk.Label(master=char_info_frame, text="Target Name:", width=15).grid(row=4, column=0)
        ttk.Label(master=char_info_frame, text="Target HP:", width=15).grid(row=5, column=0)
        ttk.Label(master=char_info_frame, text="Pos:", width=15).grid(row=6, column=0)
        ...
        ttk.Label(master=char_info_frame, textvariable=self._vars['hp'], width=25).grid(row=2, column=1)
        ttk.Label(master=char_info_frame, textvariable=self._vars['mana'], width=25).grid(row=3, column=1)
        ttk.Label(master=char_info_frame, textvariable=self._vars['target_name'], width=25).grid(row=4, column=1)
        ttk.Label(master=char_info_frame, textvariable=self._vars['target_hp'], width=25).grid(row=5, column=1)
        ttk.Label(master=char_info_frame, textvariable=self._vars['pos'], width=25).grid(row=6, column=1)
        ...
        ttk.Button(master=self, text="Save", width=10, command=self.save_config).grid(row=5, column=0)
    ...
    def save_config(self):
        ...
        print(_config)
        print(
            self.master.master.client.set_config(
