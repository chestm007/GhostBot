...
class Fairy(Locational):
    ...
    def _run(self) -> bool:
        ...
        for index, member in sorted(self._detect_team_members().items(), key=lambda i: i[1].hp_percent, reverse=True):
            if member.hp_percent < int(self.config.heal_team_threshold) and linear_distance(self._client.location, member.location) < 20:
    ...
    def _heal_self(self):
        if self._client.hp_percent < int(self.config.heal_self_threshold):
