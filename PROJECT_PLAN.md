...
- 🐾 **PET (Tamer) RECONSTRUÍDO (2026-05-27), a validar ao vivo** — `functions/petfood.py` + aba Pet refeitas (eram cascas). Sustentação: invoca no Start, **re-invoca se o pet morrer** (detecta `pet_active` a cada ciclo), alimenta a cada X min, e re-invocação periódica OPCIONAL (`spawn_interval_mins`; vazio = só quando morre). Combate do pet = aba Attack (combo). ⏳ A confirmar com o dono: o pet ataca sozinho ou precisa de tecla no combo? expira no tempo (precisa do timer)?
...
| **🐾 Pet (Tamer): invoca/alimenta/re-invoca** | ✅ RECONSTRUÍDO (2026-05-27), a validar | `petfood.py` + aba Pet. Invoca no Start, re-invoca se morrer (`pet_active`), alimenta a cada X min, re-invocar periódico opcional. Combate = aba Attack (combo). |
